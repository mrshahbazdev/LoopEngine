<x-layouts.app :title="__('app.assign_process')">
    ...
    <div class="mx-auto max-w-lg">
        <form method="POST" action="{{ route('team.assign') }}" class="space-y-6 rounded-lg bg-white p-6 shadow">
            @csrf

            <div>
                <label for="process_id" class="block text-sm font-medium text-gray-700">{{ __('app.process') }} *</label>
                ...
                @error('process_id') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
            </div>

            <div>
                <label for="user_id" class="block text-sm font-medium text-gray-700">{{ __('app.assigned_to') }} *</label>
            ...
            <div class="flex justify-end gap-3">
                <a href="{{ route('team.assignments') }}" class="rounded-md bg-white px-4 py-2 text-sm font-semibold text-gray-700 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">
                    {{ __('app.cancel') }}
                </a>
